...
- PVE tab consumes read-only Proxmox API data: node latency/status, CPU/RAM/storage, ZFS, SMART, VM/LXC running names/counts and optional per-running-workload mini-metrics. CPU/RAM/storage bars are explicitly labelled, workload mini-cards show CPU/RAM/HDD bars with totals where meaningful, and LXC workloads are labelled as `LXC` rather than `CT`. The old combined temp/storage row was removed because Proxmox CPU temperature is unavailable here and storage is already shown as its own labelled bar.
