...
- Revisit the NUT tab information hierarchy after abandoning the hold/disarm button idea. The next pass should keep the tab read-only and better explain the actual NUT model: `upsd`/driver telemetry, local `upsmon` primary shutdown automation, downstream `upsmon` secondary readiness, and reachability vs armed state.
