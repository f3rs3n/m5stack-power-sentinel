...
`proxmox` currently uses the Python stdlib Proxmox API adapter and does not add package dependencies. Home Assistant is not an installer/runtime placeholder; add future package/config dependencies here only when a module backend is introduced with tests.
